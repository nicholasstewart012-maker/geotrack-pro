...
def create_app():
    ...
    It attempts to load the real app with all dependencies.
    If that fails (ImportError, DatabaseError, etc.), it returns a 'Safe Mode' app
    that simply reports the error in JSON format.
    """
    try:
        # 1. Attempt critical imports
        ...
        # 4. Define Models & Helpers (Locally scoped to this function or imported)
        ...
        @app.get("/health")
        def health_check():
            return {"status": "ok", "timestamp": datetime.now().isoformat(), "mode": "FULL_APP"}

        @app.on_event("startup")
        def startup_seed():
            print("BACKEND STARTING UP (FULL MODE)...")
            # Verify DB Connection
            try:
                # Simple check without calling database yet
        ...
        safe_app = FastAPI(title="Safe Mode (Crash Reporter)")

        @safe_app.middleware("http")
        ...
        @safe_app.api_route("/{path_name:path}", methods=["GET", "POST", "PUT", "DELETE"])
